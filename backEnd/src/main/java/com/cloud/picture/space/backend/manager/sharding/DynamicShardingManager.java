...
public class DynamicShardingManager {
    ...
    private static final String DATABASE_NAME = "logic_db";// 配置文件中数据库的名称；
    ...
    /**
     * 获取所有动态表名，包括初始表picture和分表picture_{spaceId}
     */
    private Set<String> fetchAllPictureTableNames() {
        // 直接对所有团队空间分表（实际上线改为仅对旗舰版生效【待修改】）
        ...
        Set<String> tableNames = spaceIds.stream()
                .map(spaceId -> String.format("%s_%s", LOGIC_TABLE_NAME, spaceId))
                .collect(Collectors.toSet());
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream().map(tableName -> "yun-picture." + tableName)
                .collect(Collectors.joining(","));
        log.info("动态分表 actual-data-nodes 配置：{}", newActualDataNodes);

        ContextManager contextManager = getContextManager();
        ShardingSphereRuleMetaData ruleMetaData = contextManager
                .getMetaDataContexts()
                .getMetaData()
                .getDatabases().get(DATABASE_NAME)
                .getRuleMetaData();
            ...
            List<ShardingTableRuleConfiguration> updatedRules = ruleConfig.getTables().stream()
                    .map(oldTableRule -> {
                        if (LOGIC_TABLE_NAME.equals(oldTableRule.getLogicTable())) {
                            ShardingTableRuleConfiguration newTableRuleConfiguration
                                    = new ShardingTableRuleConfiguration(LOGIC_TABLE_NAME, newActualDataNodes);
                            newTableRuleConfiguration.setDatabaseShardingStrategy(oldTableRule.getDatabaseShardingStrategy());
                            newTableRuleConfiguration.setTableShardingStrategy(oldTableRule.getTableShardingStrategy());
                            newTableRuleConfiguration.setKeyGenerateStrategy(oldTableRule.getKeyGenerateStrategy());
                            newTableRuleConfiguration.setAuditStrategy(oldTableRule.getAuditStrategy());
    ...
    private ContextManager getContextManager() {
        try (ShardingSphereConnection connection = dataSource.getConnection().unwrap(ShardingSphereConnection.class)) {
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "获取ShardingSphere ContextManager失败:" + e);
        }
    }
